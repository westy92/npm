...
function auditCmd (args, cb) {
  ...
  return Bluebird.all([
    ...
    if (args[0] === 'fix') {
      ...
      return Bluebird.try(() => {
        if (actions.update.length || actions.install.length) {
          log.notice('audit', 'installing', actions.install.length + actions.update.length, 'updated packages with patched vulnerabilities')
        }
        ...
        if (actions.major.length || actions.install.length) {
          return Bluebird.fromNode(cb => {
            log.verbose('audit', `running 'npm install ${actions.install.join(' ')}'`)
